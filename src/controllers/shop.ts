...
export const buyProduct = async ctx => {
    ...
    try {
        await ctx.db.query('UPDATE products SET buyer_id = ? WHERE id = ?', [
...
export const shopInfo = async ctx => {
    if (!+ctx.params.id) {
    ...
    try {
        const shopInfo = (
            await ctx.db.query('SELECT * FROM shops WHERE id = ?', [
                ctx.params.id,
            ])
        )[0][0];

        shopInfo.products = (
            await ctx.db.query('SELECT * FROM products WHERE shop_id = ?', [
                ctx.params.id,
            ])
        )[0];
...
export const createShop = async ctx => {
    ...
    if (!data.name || !data.description || !data.cost || !data.products) {
        ctx.body = {
    ...
    try {
        const newShopId = (
            await ctx.db.query(
                ...
                [data.name, data.description, data.cost, 1],
            )
        )[0].insertId;

        for (let i = 0; i < data.products.length; i++) {
            const product = data.products[i];
            const photo = ctx.request.files.photos[i] || null;
